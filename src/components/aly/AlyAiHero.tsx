...
export function AlyAiHero() {
  return (
    <section className="relative overflow-hidden bg-foreground">
      ...
      />
      <div className="relative keuda-container py-10 md:py-24">
        <div className="max-w-3xl">
          <span className="inline-flex items-center rounded-full border border-background/30 bg-foreground/40 px-2.5 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-[11px] md:text-xs font-semibold tracking-[0.2em] uppercase text-background/85 mb-3 sm:mb-5">
            KeudaPRO ÄLY
          </span>
          <h1 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold text-background mb-3 sm:mb-5 leading-[1.05] tracking-tight">
            Tekoälypätevyydet työelämään
          </h1>
          <p className="text-base sm:text-lg md:text-2xl font-semibold text-background/95 mb-2 sm:mb-4">
        ...
        <ol className="mt-6 sm:mt-10 grid gap-2 sm:gap-3 sm:grid-cols-3 max-w-4xl">
          {AI_LEVELS.map((level, i) => (
            <li key={level.id} className="relative">
              <button
                type="button"
                onClick={() => scrollToId(level.anchor)}
                className="group w-full h-full text-left rounded-xl border-2 border-background/25 bg-foreground/50 hover:bg-foreground/70 hover:border-background/60 hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-background p-3 sm:p-5 transition-all duration-300 shadow-lg shadow-black/20"
              >
                <span className="inline-flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-background text-foreground text-sm sm:text-base font-extrabold mb-2 sm:mb-3 shadow-md">
                  {i + 1}
                </span>
                <span className="block text-sm sm:text-base md:text-lg font-bold text-background">
                  {level.name}
                </span>
                <span className="block text-xs sm:text-sm text-background/85 mt-0.5">{level.promise}</span>
                <span className="mt-2 sm:mt-3 inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-background/70 group-hover:text-background transition-colors">
                  Lue lisää
        ...
        <div className="mt-6 sm:mt-10 flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3">
          ...
          <Button
            size="lg"
            variant="outline"
            className="border-2 border-background/50 text-background hover:bg-background hover:text-foreground bg-foreground/30 h-auto whitespace-normal py-2 sm:py-3 text-center text-sm sm:text-base"
            onClick={() => {
